...
- 有向无环图（DAG）：顶点和有向边。
- 哈希表（HashMap）：Key/Value键值对。Key是唯一的。
...
- keyBy的作用：
  - 指定数据的key。
  - 根据key计算出数据要去下游reduce算子的哪一个并行子任务。
  - 将数据路由到下游算子reduce的对应的并行子任务中。
- 相同key的数据一定会路由到下游reduce算子的同一个并行子任务中。
- 不同key的数据也可能路由到下游reduce算子的同一个并行子任务中。
...
算子的每一个并行子任务都有自己的<span style="color:red">生命周期</span>。
...
- AggregateFunction<IN, ACC, OUT>
  ...
  - getResult：窗口闭合时触发执行，发出聚合结果，返回值是将要发送的聚合结果
...
> 滚动窗口大小是1天，但想每隔10秒钟计算一次窗口中的统计指标，怎么办？
...
- CONTINUE：什么都不做
- FIRE：触发窗口的计算（后面的process/aggregate）
- PURGE：清空窗口中的元素
- FIRE_AND_PURGE：触发窗口计算并清空窗口中的元素

Trigger中的核心方法

- onElement：每来一条数据调用一次，返回值类型是TriggerResult。
- onProcessingTime：处理时间定时器，返回值类型是TriggerResult。
- onEventTime：事件时间定时器，返回值类型是TriggerResult。
- clear：窗口闭合时触发调用，返回值类型是void。
...
<span style="color:red">由于事件时间的世界里面没有时钟</span>，所以我们需要通过编程来为事件时间的世界提供时钟，叫做<span style="color:red">水位线（逻辑时钟）</span>。
...
4. 算子的每个并行子任务会维护自己的水位线（逻辑时钟），当接收到上游发送过来的水位线事件时，更新自己的水位线（逻辑时钟）。
...
8. ProcessWindowFunction并行子任务的水位线 $\ge$ 事件时间窗口的结束时间 - 1毫秒，触发窗口计算。
9. KeyedProcessFunction并行子任务的水位线 $\ge$ 事件时间定时器的时间戳，触发定时器执行。
...
- 合流：
  ...
  - 第二步：选择水位线数组中的最小的水位线来更新并行子任务的水位线（逻辑时钟）。
...
- 使用迟到元素<span style="color:red">更新</span>窗口计算结果。也就是当水位线到达窗口结束时间的时候，触发窗口计算，但不销毁窗口，而是选择再等待迟到元素一段时间。
...
- connect
  - 只能合并<span style="color:red">两</span>条流
  - 两条流的元素的类型可以不一样
  - DataStream API
...
- 检查点分界线事件到达哪一个并行子任务，就对哪一个并行子任务的状态做快照。和水位线类比：水位线事件到达哪一个并行子任务，就更改哪一个并行子任务的水位线。
...
- 分界线对齐：检查点分界线向下游传递，sum任务会等待所有输入分区的检查点分界线到达，才会做快照。
...
- 对于每个检查点，Sink任务会启动一个事务（下游设备的事务，比如MySQL，Kakfa）
...
- source—FlinkKafkaConsumer（实现了CheckpointedFunction接口）作为source，可以将偏移量保存下来，如果后续任务出现了故障，恢复的时候可以由连接器重置偏移量，重新消费数据，保证一致性。
- sink—FlinkKafkaProducer（实现了TwoPhaseCommitSinkFunction接口）作为sink。
...
4. 作业管理器收到所有并行子任务的通知，向所有的并行子任务广播确认信息，表示检查点完成。
