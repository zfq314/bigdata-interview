...
```
数据源经过各种数据集成和加工处理后，通常会入库到实时数仓 Doris 和离线湖仓（Hive, Iceberg, Hudi）
报表分析
        实时看板 （Dashboards）
        面向企业内部分析师和管理者的报表
...
wget https://mirrors.tuna.tsinghua.edu.cn/apache/doris/1.1/1.1.4-rc01/apache-doris-fe-1.1.4-bin.tar.gz --no-check-certificate 
```
