...
where  rq='2022-07-30' and  plmc='饰品' and ckmc='镶嵌Q柜' and wdmc='深圳展厅' and jsmc='千足金';

update T_KA_ZTKCRZZ set qcjs=qmjs+85,qcjz=qmjz+200.51,qcje=qmje+19364 
where  rq='2022-07-31' and  plmc='饰品' and ckmc='镶嵌Q柜' and wdmc='深圳展厅' and jsmc='千足金';
...
-----------------------------------------------------------------------------------------------------------------
select rq ,sum(jz) from (
select DATE(examine_time) as rq,sum(total_gold_weight) as jz from t_receive where supplier_type=0 -- and DATE(examine_time)='2022-07-12' 
and  genus_name <> '金料' and supplier_source like '福州%'  and receive_code not like 

and `status`=0 GROUP BY DATE(examine_time)    
union all 
select DATE(approve_time)as  rq ,sum(total_weight)  as jz from t_bf_gold_transfer_in where -- ='2022-07-12' 
  transfer_out_showroom like '福州%' and approve_status=1
and STATUS=0 and  genus_name <> '金料'    GROUP BY DATE(approve_time)   
)t
GROUP BY rq
;


select  rq,sum(jz) from t_ka_splsz 
where swlx='调拨入库' and  sffx='收' and (chf like '福州工厂%' or chf like '福州K金工厂%' or chf like '福州镶嵌工厂%')  and djm<>'单件调拨入库单' 
...
temp10 as(
select '福州工厂千足' as mc, sum(total_weight) as zl, sum(ifnull(total_price, 0)) as je, '' as bz /*into temp10*/
 from t_bf_gold_transfer_in where transfer_out_showroom = '福州工厂' and transfer_in_showroom = v_wdmc 
and date(approve_time) = v_rq and approve_status = 1 -- 审核状态 1、已审核  0、未审核
   and status = 0 -- 是否作废 0 正常 1 作废
...
-- 万足
temp110 as(
select '福州工厂万足' as mc, sum(total_weight) as zl, sum(ifnull(total_price, 0)) as je, '' as bz /*into temp110*/ 
from t_bf_gold_transfer_in where transfer_out_showroom = '福州工厂' and transfer_in_showroom = v_wdmc 
and date(approve_time) = v_rq and approve_status = 1 -- 审核状态 1、已审核  0、未审核
   and status = 0 -- 是否作废 0 正常 1 作废
    and  genus_name <> '金料' and purity_name = '金9999'
...
-- 五九
temp1100 as(
select '福州工厂五九金' as mc, sum(total_weight) as zl, sum(ifnull(total_price, 0)) as je, '' as bz /*into temp1100*/ 
from t_bf_gold_transfer_in where transfer_out_showroom = '福州工厂' and transfer_in_showroom = v_wdmc 
and date(approve_time) = v_rq and approve_status = 1 -- 审核状态 1、已审核  0、未审核
   and status = 0 -- 是否作废 0 正常 1 作废
...
-- 古法金
temp11000 as(
select '福州工厂古法金' as mc, sum(total_weight) as zl, sum(ifnull(total_price, 0)) as je, '' as bz /*into temp11000*/ 
from t_bf_gold_transfer_in where transfer_out_showroom = '福州工厂' and transfer_in_showroom = v_wdmc 
and date(approve_time) = v_rq and approve_status = 1 -- 审核状态 1、已审核  0、未审核 
   and status = 0 -- 是否作废 0 正常 1 作废
   and  genus_name <> '金料' and purity_name = '古法金'
), 

-- 足金(5g)
temp11001 as(
select '福州工厂足金(5g)' as mc, sum(total_weight) as zl, sum(ifnull(total_price, 0)) as je, '' as bz /*into temp11001*/
from t_bf_gold_transfer_in where transfer_out_showroom = '福州工厂' and transfer_in_showroom = v_wdmc 
and date(approve_time) = v_rq and approve_status = 1 -- 审核状态 1、已审核  0、未审核 
   and status = 0 -- 是否作废 0 正常 1 作废
and  genus_name <> '金料' and (purity_name = '足金(5g)' or technology_type = '5g')
), 

-- 古法金
temp11002 as(
select '福州工厂古法万足金' as mc, sum(total_weight) as zl, sum(ifnull(total_price, 0)) as je, '' as bz /*into temp11002*/
from t_bf_gold_transfer_in where transfer_out_showroom = '福州工厂' and transfer_in_showroom = v_wdmc 
and date(approve_time) = v_rq and approve_status = 1 -- 审核状态 1、已审核  0、未审核 
   and status = 0 -- 是否作废 0 正常 1 作废
   and  genus_name <> '金料' and purity_name = '古法万足金'
), 

-- 古法999.99
temp11003 as(
select '福州工厂古法999.99' as mc, sum(total_weight) as zl, sum(ifnull(total_price, 0)) as je, '' as bz /*into temp11003*/
from t_bf_gold_transfer_in where transfer_out_showroom = '福州工厂' and transfer_in_showroom = v_wdmc 
and date(approve_time) = v_rq and approve_status = 1 -- 审核状态 1、已审核  0、未审核 
   and status = 0 -- 是否作废 0 正常 1 作废
and  genus_name <> '金料' and purity_name = '古法999.99'
), 

-- 足金(无氰)
temp11004 as(
select '福州工厂足金(无氰)' as mc, sum(total_weight) as zl, sum(ifnull(total_price, 0)) as je, '' as bz /*into temp11004*/
from t_bf_gold_transfer_in where transfer_out_showroom = '福州工厂' and transfer_in_showroom = v_wdmc 
and date(approve_time) = v_rq and approve_status = 1 -- 审核状态 1、已审核  0、未审核
   and status = 0 -- 是否作废 0 正常 1 作废
 and  genus_name <> '金料' and purity_name = '足金(无氰)'
), 

temp22004 as(
 SELECT
'福州工厂' as mc,
sum(IFNULL(total_gold_weight,0)) as zl,
sum(IFNULL(total_fee,0)) as je,
'' as bz 
FROM
 t_receive r 
WHERE
 r.supplier_type = 0
 and r.supplier_source like '福州%' 
 AND r.STATUS = 0 
 and  date(r.examine_time) = v_rq 
 and r.showroom_name=v_wdmc  
 and r.genus_name <> '金料' 
), 
...
-----------------------------------------------------------------------------------------------------------------
select  djh,sum(jz) from t_ka_splsz 
where swlx='调拨入库' and  sffx='收' and (chf like '福州工厂%' or chf like '福州K金工厂%' or chf like '福州镶嵌工厂%')  and djm<>'单件调拨入库单' and rq='2022-08-12'
GROUP BY djh ORDER BY djh ;


select sum(total_gold_weight) as jz from t_receive where supplier_type=0 and DATE(examine_time)='2022-08-12' 
and  genus_name <> '金料' and supplier_source like '福州%' 
and receive_code in ('SJDRD609101',
'SJDRD612681',
'SJDRD612912',
'SJDRD612914',
'SJDRD612916',
...
select sum(total_weight) as jz from t_bf_gold_transfer_in where DATE(approve_time) ='2022-08-12' and 
  transfer_out_showroom like '福州%' and approve_status=1
and STATUS=0 and  genus_name <> '金料'   
 and transfer_in_code in ('SJDRD609101',
'SJDRD612681',
'SJDRD612912',
'SJDRD612914',
'SJDRD612916',
...
-----------------------------------------------------------------------------------------------------------------
MYSQL 获取字段
 SELECT CONCAT(column_name,',') AS field  FROM information_schema.COLUMNS WHERE  table_schema='decent_cloud' and table_name = "rpquerysalesstoragedata_modify_result4" ORDER BY ORDINAL_POSITION asc
...
-----------------------------------------------------------------------------------------------------------------
 SELECT
                       ...
                       FROM
                        ( SELECT column_name AS field,data_type AS type FROM information_schema.COLUMNS WHERE  table_schema='decent_cloud' and table_name = "t_cpbszh_log_detail" ORDER BY ORDINAL_POSITION asc ) t
                 #### 获取字段，写脚本的时候方便
                         SELECT CONCAT(column_name,',') AS field  FROM information_schema.COLUMNS WHERE  table_schema='decent_cloud' and table_name = "t_cpbszh_log_detail" ORDER BY ORDINAL_POSITION asc
 ...
 datax.py -p"-Dtargetdir= /sz_decent_cloud/t_ka_splsz" /opt/module/datax/job/import/gmall.activity_info.json
...
create table if not exists t_ka_splsz
(
 id  int,
djm string,
djh string,
...
parent_customer_identity  string,
RHF string,
CHF string,
bz  string,
BZGF  decimal(18,3),
...
MZ  decimal(18,3),
czzt  string,
tech_purity_code  string,
tech_purity_name  string,
bzh string
)
    row format delimited fields terminated by '\t'
    location '/sz_decent_cloud/t_ka_splsz'
;
